...
if((isset($_POST["id"]) && !empty($_POST["id"])) && isset($_POST["nombre"]) && isset($_POST["codigo"]) && isset($_POST["precioCliente"]) && isset($_POST["precioEmpleado"]) && isset($_POST["categoria"]) && isset($_POST["precioCompra"]) && isset($_POST["gananciaEmpleado"]) && isset($_POST["gananciaCasa"])){
    ...
    $gananciaCasa = $_POST["gananciaCasa"];
        try{
        $stmt = $conn->prepare("UPDATE Productos SET Nombre_de_Producto =:nombre, Codigo_de_Producto =:codigo, Precio_Empleado =:precioEmpleado, Ganancia_Empleado =:gananciaEmpleado, Ganancia_Casa =:gananciaCasa, Precio_Clientes =:precioCliente, Categoria =:categoria, Precio_de_Compra =:precioCompra, Porcentaje_Casa =:porcentajeCasa, Porcentaje_Empleada =:porcentajeEmpleada WHERE ID_Producto =:id");
        $result = $stmt->execute([
            "nombre" => $nombre,
            "codigo" => $codigo,
