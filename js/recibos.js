        ...
        $.post("Backend/select-single-product.php", {id}, function(response){
            try{
                ...
                if(template == undefined || template == "" || template == null || template.includes('<th>') !== true ){
                    ...
                    let nuevoTotalFactura = parseFloat((parseFloat(totalFactura) + parseFloat(res[0].cantidad*res[0].precioCompra))).toFixed(2)
                    $('#totalFactura').text((nuevoTotalFactura));
                }
                $('#idProducto').val("");
                ...
                $('.campo').toArray().forEach(function(field){
...
$(document).on('change', '.campo', function(){
    ...
    if(campo.includes('precioCompra')){
        ...
        $(`#total${id}`).text(parseFloat(nuevoPrecioProducto).toFixed(2));
        $('#totalFactura').text(parseFloat(nuevoPrecioTotal).toFixed(2));
    ...
    aArray.forEach(input =>{
        ...
        if(!isNaN(input[0])){
            ...
            if(typeof(array[j]) == 'undefined'){
                array.push([]);
                array[j].push([$(`#${input[1].id}`).attr("data-producto"),$(`#${input[1].id}`).val(),$(`#${input[1].id}`).attr("data-id"), $('#username').val(), $('#proveedor').val(), $('#numFactura').val()]);
                if(i==3){
                    j=0;
                    i = 1;
                }else{i++}
