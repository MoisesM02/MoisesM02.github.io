$(function(){
    ...
    $("#buscar").on("click", function (e) {
		...
		if (id != null || id !== undefined || !id) {
			$.post("Backend/seleccionar-producto-devolucion.php", data, function (response) {
				try {
                    let res = JSON.parse(response);
                    console.log(res);
		...
		aArray.forEach((input) => {
			...
			Swal.fire({
				...
				if (result.isConfirmed) {
					$.post("Backend/devolverventa.php", { array }, function (response) {
						Swal.fire(response);
					});
